...
<div class="modal fade" id="empAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
	...
	<div class="container">
		...
		<div class="row">
			<!-- 分页信息 -->
			<div class="col-md-6" id="page_info_area">当前第页，总页码数:,总记录数:</div>
			<!-- 分页条 -->
	...
	<script type="text/javascript">
		...
		$("#emp_add_modal_btn").click(function(){
		 ...
		 function validate_add_form(){
			 ...
			 var regName = /(^[a-zA-Z0-9_-]{6,18}$)|(^[\u2E80-\u9FFF]{2,5})/;
			 if(!regName.test(empName)){
				 show_validate_msg("#empName_add_input","error","用户名可设置为2-5位中文或6-18位英文或数字结合");
				 return false;
			 }else{
		 ...
		 function show_validate_msg(ele,status,msg){
			 //清除当前元素的校验状态
			 $(ele).parent().removeClass("has-success has-ereor");
			 $(ele).next("span").text("");
			 if("success"==status){
				 $(ele).parent().addClass("has-success");
				 $(ele).next("span").text(msg);
			 }else if("error" == status){
				 $(ele).parent().addClass("has-ereor");
				 $(ele).next("span").text(msg);
			 }
		 }
